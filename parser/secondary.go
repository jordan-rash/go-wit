...
func (p *Parser) parseResultShape() *ast.ResultShape {
	...
	case token.KEYWORD_STRING, token.KEYWORD_BOOL, token.KEYWORD_CHAR,
		...
		token.KEYWORD_S8, token.KEYWORD_S16, token.KEYWORD_S32, token.KEYWORD_S64,
		token.KEYWORD_U8, token.KEYWORD_U16, token.KEYWORD_U32, token.KEYWORD_U64,
		token.IDENTIFIER, token.OP_UNDERSCORE:

		p.nextToken()

		if p.curToken.Literal == token.OP_UNDERSCORE {
			rs.OkValue = nil
		} else {
		...
		case token.OP_COMMA:
			...
			if p.curToken.Literal == token.KEYWORD_LIST {
				rs.ErrValue = &ast.Child{Token: p.curToken, Value: p.parseListShape()}
			} else if p.curToken.Literal == token.KEYWORD_OPTION {
				rs.ErrValue = &ast.Child{Token: p.curToken, Value: p.parseOptionShape()}
			} else if p.curToken.Literal == token.KEYWORD_RESULT {
		...
		case token.OP_BRACKET_ANGLE_RIGHT:
			break
		}
